"""
GDPR Document Converter with Intelligent Metadata Integration

This module handles the conversion of enhanced GDPR JSON chunks into LangChain Document 
objects while applying the sophisticated metadata flattening approach.

We take your carefully structured data and transform it into a format that works with vector databases while preserving all the 
sophisticated capabilities:
- Input language: enhanced JSON with complex nested metadata
- Output language: LangChain Documents with flattened but complete metadata

The converter creates "bilingual documents" that work efficiently with vector databases
while preserving all the information needed for precise legal citations.
"""
...
from gdpr_metadata_flattener import GDPRMetadataFlattener


class GDPRDocumentConverter:
    """
    Converts enhanced GDPR JSON chunks to LangChain Documents with intelligent metadata flattening.
    
    This class represents the core solution to the vector database constraint challenge.
    We transform sophisticated nested metadata into a format that Chroma can store
    while preserving all the information the enhanced agent needs for precise citations.
    ...
    (complex nested structures) simultaneously.
    """
    
    def __init__(self, metadata_flattener: GDPRMetadataFlattener, logger: logging.Logger):
        """
        ...
        self.logger = logger
        self.logger.info("GDPR Document Converter initialized with intelligent metadata flattening")
    ...
    def convert_chunks_to_documents(self, chunks: List[Dict[str, Any]], 
        ...
        Args:
            chunks: List of enhanced JSON chunks from GDPR processing
        ...
        """
        self.logger.info("Starting conversion of enhanced GDPR JSON chunks to LangChain Document objects...")
        ...
        for i, chunk in enumerate(chunks):
            try:
                ...
                self.logger.error(f"Error converting chunk {i}: {str(e)}")
                continue
    ...
    def _validate_chunk_content(self, content: str, chunk_index: int) -> bool:
        ...
        This ensures we don't create empty or invalid documents that would
        cause issues in the vector database.
        """
        if not content or not content.strip():
            self.logger.warning(f"Empty content in GDPR chunk {chunk_index}, skipping...")
            return False
        return True
    ...
    def _build_enhanced_metadata(self, chunk_metadata: Dict[str, Any], 
        ...
        This creates the foundation metadata that includes both GDPR-specific information
        and processing context needed for sophisticated analysis.
        """
        # Track chunk type for statistical analysis
        chunk_type = chunk_metadata.get('type', 'unknown')
        
        # Create the comprehensive metadata structure
        enhanced_metadata = {
            # Basic GDPR document structure (always simple values for database compatibility)
            'type': chunk_type,
    ...
    def _apply_metadata_flattening(self, chunk_metadata: Dict[str, Any], 
        ...
        We take sophisticated nested metadata and apply the flattening algorithm
        to make it compatible with vector databases while preserving all the information needed for precise citations.
        """
        ...
        if article_structure:
            ...
            # Track that this chunk has enhanced structure
            self.conversion_stats['enhanced_structure_count'] += 1
            
            self.logger.debug(f"Enhanced GDPR chunk {chunk_index}: Article {enhanced_metadata.get('article_number', 'N/A')} "
                           f"with {flattened_structure.get('paragraph_count', 0)} paragraphs, "
                           f"complexity: {flattened_structure.get('complexity_level', 'unknown')}")
        else:
            # No enhanced structure - set basic indicators for GDPR compatibility
    ...
    def _update_conversion_statistics(self, chunk_metadata: Dict[str, Any], 
        ...
        These statistics help track the effectiveness of the conversion process
    ...
    def _log_sample_conversion(self, enhanced_metadata: Dict[str, Any], 
        ...
        for the first few documents, helping identify any issues early.
        """
        ...
        self.logger.info(f"Sample GDPR chunk {chunk_index}: {chunk_type} - "
    ...
    def _log_conversion_results(self) -> None:
        ...
        self.logger.info("=" * 60)
        self.logger.info("ENHANCED GDPR METADATA CONVERSION COMPLETED")
        self.logger.info("=" * 60)
        ...
        if stats['chunk_types']:
            self.logger.info("GDPR chunk type distribution:")
        ...
        if stats['complexity_levels']:
            self.logger.info("GDPR complexity level distribution:")
        ...
        if stats['successful_conversions'] > 0:
            ...
            self.logger.info(f"GDPR enhanced structure rate: {enhancement_rate:.1f}%")
        
        # Log metadata flattening summary
    ...
    def get_conversion_statistics(self) -> Dict[str, Any]:
        ...
        # Add metadata flattening statistics
...
def create_gdpr_document_converter(metadata_flattener: GDPRMetadataFlattener, 
                                  logger: logging.Logger) -> GDPRDocumentConverter:
    """
    Factory function to create a configured GDPR document converter.
    
    This provides a clean interface for creating converter instances with
    proper dependency injection of the metadata flattener and logger.
    """
    return GDPRDocumentConverter(metadata_flattener, logger)