...
def load_enhanced_json(file_path):
    """
    Load the enhanced Polish law JSON file with sophisticated structural metadata.
    
    This function reads your carefully crafted JSON that contains both content
    and rich structural metadata about legal document organization. We validate
    the structure to ensure it contains the enhanced metadata we expect.
    """
    logger.info(f"Loading enhanced JSON file from: {file_path}")
    ...
    try:
        ...
        logger.error(f"Error loading enhanced JSON file: {str(e)}")
        raise

def flatten_article_structure(article_structure):
    """
    Intelligently flatten complex article structure metadata for vector database compatibility.
    
    This function solves the core challenge: your sophisticated nested metadata structure
    is perfect for representing legal document organization, but vector databases can only
    store simple values. We extract the essential information as simple key-value pairs
    while preserving the complete structure for full access when needed.
    
    This approach demonstrates "lossless flattening" - we don't lose any information,
    we just transform it into a format that works within technical constraints.
    """
    logger.debug("Flattening complex article structure metadata...")
    ...
    if not article_structure or not isinstance(article_structure, dict):
        logger.debug("No complex structure to flatten")
        return flattened
    ...
    try:
        ...
        for para_key, para_info in paragraphs_info.items():
            if isinstance(para_info, dict):
                ...
                if para_info.get('has_sub_paragraphs', False):
                    ...
                    # Collect numbering styles used in this document
                    style = para_info.get('numbering_style', '')
                    if style:
                        numbering_styles.add(style)
                    
                    # Track sub-paragraph complexity
        ...
        # Determine complexity level for quick filtering by agents
        ...
        logger.debug(f"Flattened structure: {flattened['paragraph_count']} paragraphs, "
        ...
        logger.warning(f"Error flattening article structure: {e}")
...
def create_documents_from_enhanced_chunks(chunks, source_metadata):
    """
    Convert enhanced JSON chunks to LangChain Document objects with intelligent metadata flattening.
    
    This function represents the core solution to the vector database constraint challenge.
    We transform your sophisticated nested metadata into a format that Chroma can store
    while preserving all the information your enhanced agent needs for precise citations.
    
    Think of this as creating a bilingual document - it speaks both the "vector database
    language" (simple key-value pairs) and the "sophisticated legal analysis language"
    (complex nested structures) by storing both representations.
    """
    logger.info("Converting enhanced JSON chunks to LangChain Document objects...")
    logger.info("Implementing intelligent metadata flattening for vector database compatibility...")
    ...
    for i, chunk in enumerate(chunks):
        try:
            ...
            # Track chunk types for statistical analysis
            chunk_type = metadata.get('type', 'unknown')
            processing_stats['chunk_types'][chunk_type] = processing_stats['chunk_types'].get(chunk_type, 0) + 1
            
            # Start building enhanced but flattened metadata
            # We preserve all the essential information while ensuring compatibility
            enhanced_metadata = {
                # Basic document structure (always simple values)
                ...
                # Document-level context for legal research
            ...
            # Handle enhanced article structure with intelligent flattening
            article_structure = metadata.get('article_structure', {})
            if article_structure:
                # Apply our sophisticated flattening algorithm
                ...
                # Track statistics about enhanced structures
                ...
                logger.debug(f"Enhanced chunk {i}: Article {enhanced_metadata.get('article_number', 'N/A')} "
                           f"with {flattened_structure.get('paragraph_count', 0)} paragraphs, "
                           f"complexity: {complexity}")
            else:
                # No enhanced structure - set basic indicators
            ...
            if i < 3:
                ...
                logger.info(f"Sample chunk {i}: {chunk_type} - "
    ...
    logger.info("=" * 60)
    logger.info("ENHANCED METADATA PROCESSING COMPLETED")
    logger.info("=" * 60)
    ...
    logger.info("Complexity level distribution:")
    ...
    logger.info(f"Enhanced structure rate: {enhancement_rate:.1f}%")
    
    return docs

def embed_documents_with_flattened_metadata(docs):
    """
    Embed documents into the Chroma vector store using flattened metadata.
    
    This function handles the embedding process with our new flattened metadata approach.
    The flattened metadata should now be compatible with Chroma's constraints while
    preserving all the sophisticated structural information your system needs.
    
    We include comprehensive error handling and logging to ensure you can monitor
    the success of our metadata flattening solution.
    """
    logger.info("Creating embeddings and storing in vector database with flattened metadata...")
    ...
    if docs:
        logger.info("Testing metadata compatibility with sample document...")
        try:
            test_doc = docs[0]
            # Attempt to add one document to test our flattening approach
            db.add_documents([test_doc])
            logger.info("✅ Metadata compatibility test successful - flattening approach works!")
            ...
            logger.error(f"❌ Metadata compatibility test failed: {e}")
            logger.error("This suggests our flattening approach needs adjustment")
            raise Exception(f"Metadata flattening failed compatibility test: {e}")
    ...
    for i in tqdm(range(0, len(docs), batch_size), desc="Embedding enhanced documents"):
        ...
        logger.info(f"Batch {batch_num} sample: {', '.join(sample_articles)}")
        ...
        while retry_count < max_retries:
            try:
                ...
                if "metadata" in str(e).lower():
                    ...
                    logger.error("This suggests our flattening approach may have missed some complex metadata")
    ...
    logger.info("=" * 60)
    logger.info("ENHANCED EMBEDDING PROCESS COMPLETED")
    logger.info("=" * 60)
    ...
    try:
        ...
        # Test retrieval of enhanced metadata
        if final_count > 0:
            ...
            if test_docs:
                ...
                logger.info(f"Verification: Retrieved document has enhanced structure: {'✓' if has_enhanced else '✗'}")
                
                if has_enhanced:
                    logger.info(f"Enhanced metadata preserved: paragraph_count={test_metadata.get('paragraph_count', 0)}, "
    ...
    if metadata_errors > 0:
        logger.warning(f"⚠️  {metadata_errors} metadata errors occurred - review flattening approach")
    else:
        logger.info("✅ No metadata errors - flattening approach completely successful!")
    
    return db

def save_enhanced_processing_summary(docs, output_path):
    """
    Save a comprehensive summary of enhanced document processing.
    
    This summary helps you understand how well the enhanced metadata flattening
    worked and provides statistics about the structural complexity of your
    processed legal documents.
    """
    logger.info(f"Saving enhanced processing summary to: {output_path}")
    ...
    # Analyze enhanced metadata in processed documents
    ...
    logger.info(f"Enhanced processing summary saved:")
...
def process_enhanced_polish_law():
    """
    Main function to process enhanced Polish law JSON with sophisticated metadata flattening.
    
    This represents the complete solution to the vector database constraint challenge.
    We take your enhanced JSON structure and transform it into a format that works
    within technical limitations while preserving all the sophisticated capabilities
    your legal analysis system needs.
    """
    start_time = time.time()
    logger.info("=" * 80)
    logger.info("STARTING ENHANCED POLISH LAW PROCESSING WITH METADATA FLATTENING")
    logger.info("=" * 80)
    
    # Updated path to look for your enhanced JSON structure
    polish_law_json_file = os.path.join(PROCESSED_DIR, "polish_law_final_manual.json")
    
    if not os.path.exists(polish_law_json_file):
        # Also check RAW_DIR as backup location
        backup_path = os.path.join(RAW_DIR, "polish_law_final_manual.json")
        if os.path.exists(backup_path):
            polish_law_json_file = backup_path
            logger.info(f"Using backup location: {backup_path}")
        else:
            logger.error(f"Enhanced JSON file not found in either location:")
            logger.error(f"  Primary: {os.path.join(PROCESSED_DIR, 'polish_law_final_manual.json')}")
            logger.error(f"  Backup: {backup_path}")
            raise FileNotFoundError(f"Required enhanced JSON file not found")
    
    try:
        # Step 1: Load enhanced JSON with structural validation
        logger.info("STEP 1: Loading enhanced JSON with structural validation...")
        data, chunks = load_enhanced_json(polish_law_json_file)
        source_metadata = data.get('document', {}).get('metadata', {})
        
        # Step 2: Convert chunks with intelligent metadata flattening
        logger.info("STEP 2: Converting chunks with intelligent metadata flattening...")
        docs = create_documents_from_enhanced_chunks(chunks, source_metadata)
        
        if not docs:
            logger.error("No valid documents created from enhanced chunks")
            return None
        
        # Step 3: Embed documents with flattened but complete metadata
        logger.info("STEP 3: Embedding documents with flattened metadata...")
        db = embed_documents_with_flattened_metadata(docs)
        ...
        logger.info(f"Vector store location: {PL_DB_DIR}")
        ...
        execution_time = time.time() - start_time
        logger.error("=" * 80)
        logger.error("ENHANCED POLISH LAW PROCESSING FAILED")
...
if __name__ == "__main__":
    try:
        process_enhanced_polish_law()
        logger.info("Enhanced processing completed successfully!")
    except Exception as e:
        logger.error(f"Fatal error in enhanced processing: {str(e)}")
        exit(1)